...
export default function AsideTable({
  ...
  React.useEffect(() => { setPage(1); }, [search, data]);
    ...
    <>
      ...
      >
        ...
        <button
          ...
          tabIndex={0}
        >
          <span className="text-3xl font-bold leading-none"><X size={24}/></span>
        </button>
        {/* Panel Content */}
            ...
            {pagedData.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr>
                    {columns.map(col => (
                      <th key={col.key} className="py-2 px-4 text-left text-gray-400 font-semibold border-b border-gray-700/50">{col.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {pagedData.map((row, idx) => (
                    <tr key={row.id || idx} className="border-b border-gray-700/50 hover:bg-gray-700/50 transition-colors even:bg-gray-800/50">
                      {columns.map(col => (
                        <td key={col.key} className="py-3 px-4 text-gray-300 truncate">{row[col.key]}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
