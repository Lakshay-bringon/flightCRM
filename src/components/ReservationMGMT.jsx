...
function ReservationMGMT() {
  ...
  if (showConfirmation) {
    return <BookingConfirmation initialData={{ transactionType: selectedType }} onBack={() => setShowConfirmation(false)} />
  }
        ...
        {Object.entries(TRANSACTION_TYPES).map(([key, value]) => (
          ...
          >
            ...
            <p className="text-xs text-gray-400 flex-grow">
              Create new {key.split('_').map(word => word.toLowerCase()).join(' ')}
            </p>
