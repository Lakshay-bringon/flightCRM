...
function IPSetting() {
  ...
  const [ipList, setIpList] = useState([
    ...
    { ip: '2001:0db8:85a3:0000:0000:8a2e:0370:7334', desc: 'IPv6 Blocked', status: 'blocked' },
  ...
  const handleToggleStatus = (ip) => {
    setIpList(list => list.map(item =>
      item.ip === ip ? { ...item, status: item.status === 'allowed' ? 'blocked' : 'allowed' } : item
    ));
  };
  ...
  const filteredIPs = ipList.filter(ipObj => {
    ...
    const matchesFilter =
      filter === 'all' || ipObj.status === filter;
    ...
    <div className="max-w-7xl mx-auto">
        ...
        <div className="lg:col-span-2">
          <div className="rounded-lg bg-gray-800 bg-opacity-60 backdrop-blur-lg border border-gray-700">
            <div className="p-4 border-b border-gray-700">
              <h3 className="text-base font-semibold text-white">Add IP Address</h3>
            </div>
            <div className="p-4">
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-400 mb-1">Access</label>
          ...
          <div className="rounded-lg bg-gray-800 bg-opacity-60 backdrop-blur-lg border border-gray-700">
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              ...
              <div className="relative">
                <button
                  className={`px-3 py-1 rounded-lg flex items-center gap-2 text-xs font-semibold transition-all duration-200 ${ipProtectionActive ? 'bg-green-500/20 text-green-400 hover:bg-green-500/40' : 'bg-red-500/20 text-red-400 hover:bg-red-500/40'}`}
                  onClick={() => {
                  ...
                  onMouseEnter={() => ipProtectionActive && setShowDeactivateTooltip(true)}
                  onMouseLeave={() => setShowDeactivateTooltip(false)}
                >
                  <Shield className="w-4 h-4" />
                  {ipProtectionActive ? 'IP Protection Active' : 'IP Protection Deactivated'}
                </button>
        ...
        <div className="lg:col-span-3">
          <div className="rounded-xl bg-gray-800 bg-opacity-50 backdrop-blur-lg border border-gray-700 overflow-hidden">
            <div className="p-6 border-b border-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              <h3 className="text-xl font-semibold text-white">IP Access List</h3>
              <div className="flex gap-2 items-center w-full md:w-auto">
                ...
                />
                <button
                  className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all duration-200 ${filter === 'all' ? 'bg-blue-500/20 text-blue-400' : 'bg-gray-700 text-gray-300'}`}
                  onClick={() => setFilter('all')}
                >
                  All
                </button>
                <button
                  className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all duration-200 ${filter === 'allowed' ? 'bg-green-500/20 text-green-400' : 'bg-gray-700 text-gray-300'}`}
                  onClick={() => setFilter('allowed')}
                >
                  Allowed
                </button>
                <button
                  className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all duration-200 ${filter === 'blocked' ? 'bg-red-500/20 text-red-400' : 'bg-gray-700 text-gray-300'}`}
            ...
            <div className="divide-y divide-gray-700">
              {filteredIPs.length === 0 ? (
                <div className="p-4 text-center text-gray-400 text-xs">No IPs found.</div>
              ) : (
                filteredIPs.map((ipObj, idx) => (
                  <div key={ipObj.ip + idx} className="p-4 hover:bg-gray-700/50 transition-all duration-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <Globe className="w-5 h-5 text-blue-400" />
                        <div>
                          <h4 className="text-white font-medium text-xs">{ipObj.ip}</h4>
                          <p className="text-xs text-gray-400">{ipObj.desc}</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <button
                          className={`px-2 py-0.5 text-xs rounded-full font-semibold transition-all duration-200 focus:outline-none ${ipObj.status === 'allowed' ? 'bg-green-500/20 text-green-400 hover:bg-green-500/40' : 'bg-red-500/20 text-red-400 hover:bg-red-500/40'}`}
      ...
      {showDeactivateConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-transparent backdrop-blur-[2px]" onClick={() => setShowDeactivateConfirm(false)}></div>
          <div className="relative bg-gray-800 rounded-xl shadow-2xl border border-gray-700 w-full max-w-sm mx-2 p-6 z-10 flex flex-col items-center">
            ...
            <h2 className="text-lg font-bold text-white mb-2 text-center">Deactivate IP Protection?</h2>
            <p className="text-gray-300 text-center mb-4">If you deactivate IP protection, this CRM can be accessed from anywhere. Are you sure you want to proceed?</p>
